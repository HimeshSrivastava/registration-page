...
const Registeration = () => {
  ...
  const handlebackend=()=>{
          ...
          axios.post("https://registration-page-ihac.onrender.com/register",{name:nameref.current.value,email:emailref.current.value,password:passwordref.current.value}).then((result)=>{
            console.log(result);
          }).catch((err)=>{
            console.log(err);
   } )
